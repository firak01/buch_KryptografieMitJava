...
//---------------------------------------------------------------------------
  private byte[] Haschcode(byte[] in) {
    int pos = in.length; 			// ab wann auff�llen?
    long bc = pos*8;				// DatenL�nge in Bitform
    int n512 = (pos*8+65)/512+1;		// n 512-er Bl�cke
    System.out.println("Anzahl Bytes="+pos);
    System.out.println("Anzahl Bl�cke="+n512);
    ...
    for (block=0; block<n512-1; block++) {	// alle kompletten Bl�cke
      byte2int(tmp,block*DatenLaenge,data,0,DatenLaenge/4);
      transform(data);
    }
//    
// auff�llen mit 1, gefolgt von Nullen + l�nges als 64 bit
