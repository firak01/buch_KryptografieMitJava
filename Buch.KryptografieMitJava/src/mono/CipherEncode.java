class CipherEncode {                      // zaehlt die Buchstaben
  ...
  public CipherEncode () {
    ...
    System.out.println("\n\nNach welcher Zeichenkette soll chiffriert werden?\n"+
                       "26 Buchstaben oder <Enter>, dann wird mit Random gearbeitet!)");
    System.out.println("+--------+---------+-----+");
    String ChiffreStr = IO.Satz();
    if (ChiffreStr.length() < 1)
      do {
        zeichen = (char)(Math.random()*27.0+65);
        if (ChiffreStr.indexOf((int)zeichen)==-1)
          ChiffreStr = ChiffreStr+zeichen; 
      } while (ChiffreStr.length()<26); 
    System.out.println("\n"+ChiffreStr);
    char[] Chiffre = ChiffreStr.toCharArray();
/*    
