public class SerpentDemo2 { 
  final int BlockGroesse =  16; // bytes in a data-block
  final int Runden = 32;              // nbr of Runden
...
//    for (int i=0; i<16; i++) p[i]=(byte)(Math.random()*128);
    ...
    int AnzahlBloecke = 1; //(p.length+BlockGroesse-1)/BlockGroesse; // Anzahl 64 Bit-Bl�cke
...
//----------------------------------------------------------------------------
  ...
  private int[] R (int i,int[] BDachi,int[][] KDach) { 
    int[] xored = xor128(BDachi,KDach[i]);
    ...
    if ((0 <= i) && (i <= Runden - 2))
      ...
      if (i == Runden - 1)
        BDachiPlus1 = xor128(SDachi,KDach[Runden]);
  ...
  private int[] Rinverse (int i,int[] BDachiPlus1,int[][] KDach) { 
    ...
    if ((0 <= i) && (i <= Runden - 2))
      ...
      if (i == Runden - 1)
        SDachi = xor128(BDachiPlus1,KDach[Runden]);
    int[] xored = SDachInverse(i,SDachi);
    int[] BDachi = xor128(xored,KDach[i]);
    return BDachi;
  }
  ...
  public static void main (String[] arg) { 
    SerpentDemo2 makeSerpentDemo2 = new SerpentDemo2();
  }
}
