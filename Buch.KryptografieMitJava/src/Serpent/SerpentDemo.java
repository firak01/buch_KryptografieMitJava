public final class SerpentDemo {
...
//------------------------------------------------------------------------
  public SerpentDemo() {				// Constructor encrypt	
    byte[] p=new byte[16];
    p[0]=(byte)0x80;
...
//-------------------------------------------------------------------------
  private final void LinTransE(int[] X,int[] Y) {	// encrypt
    X[0] = rotLinks(Y[0],13);
    X[2] = rotLinks(Y[2],3);
    X[1] = Y[1]^X[0]^X[2] ;
    X[3] = Y[3]^X[2]^(X[0]<<3);
  ...
  private final void LinTransD(int[] X,int[] Y) {	// decrypt
    X[2] = rotRechts(Y[2],22);
    X[0] = rotRechts(Y[0],5) ;
    X[2] = X[2]^Y[3]^(Y[1]<<7);
    X[0] = X[0]^Y[1]^Y[3];
    X[3] = rotRechts(Y[3],7);
...
//-------------------------------------------------------------------------
  public static void main (String[] arg) {
    SerpentDemo makeSerpentDemo = new SerpentDemo();
  }
}
